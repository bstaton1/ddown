...
\noindent
where the quantities $A$ and $T$ represent fish and tags, respectively in flown ($A_f$ and $T_f$) and unflown ($A_u$ and $T_u$) reaches in year $t$ and for aerial survey montoring $i$. This assumption states that the ratio of spawners per one tagged spawner is the same between flown and unflown river sections at the time of the aerial index count and the aerial telemetry flights. Equation \@ref(eq:air-expand1) and can be rearranged as:
...
(1) ignoring the intrinsic time linkage whereby brood year recruits (part of the response variable) make up the escapement for the one or more future brood years (the predictor variable), which then produce the future recruits (response variables) and 
...
\noindent
In order to link $R_{y,j}$ with calendar year observations of escapement from each substock, $R_{y,j}$ will be allocated to calendar year runs:
...
All parameter estimation was conducted in the Bayesian framework using JAGS [@plummer-2017] implemented using R [@r-cite] and the R package `{jagsUI}` [@r-jagsUI] for interfacing with JAGS from R, as well as the `{coda}` [@r-coda] package and a package of my own creation `{codaTools}` [@r-codaTools].
