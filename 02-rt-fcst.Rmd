...
\noindent
for each $i = 0, 1, ..., n - \Delta_{D,min} - j$ and $j = 0, 1, ..., n - \Delta_{D,min}$. Windows with $j > n - \Delta_{D,min}$ would contain fewer than $\Delta_{D_min}$ days. After evaluating all windows, the single window with the best predictive performance was used to obtain the forecast predictor variable for that data source (i.e., PDO _versus_ air temperature). As an example, consider the following inputs: 
