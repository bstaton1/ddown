...
simple average over a wide spatial area {[}e.g., mundy-evenson-2011{]}
...
sliding window algorithm \citep{vandepol-etal-2016}, was implemented to
...
sliding window algorithm was implemented is provided in Appendix
...
window analyses \citep[\texttt{\{climwin\}};][]{bailey-vandepol-2016},
the package can currently only assess candidate windows using Akiake's
Information Criterion (AIC) or \(k\)-fold cross-validation. Because the
more specialized forecasting cross-validation technique was required for
this analysis, custom R code was written to select the optimal window
for each variable (Appendix \protect\hyperlink{appendix-a}{A}).
